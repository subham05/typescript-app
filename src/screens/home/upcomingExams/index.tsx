...
import { useGetUpcomingExamListMutation } from "src/store/services/home";
...
const UpcomingExams: React.FC<UpcomingEventsProps> = () => {
  ...
  const [getUpcomingExamList, { data, isLoading }] =
    useGetUpcomingExamListMutation();
  useEffect(() => {
    getUpcomingExamList({ filter: dropdownValue, date: date });
  }, [dropdownValue, date]);
  ...
  return (
    <Container noSpacing>
      <Header
        scroll={scrollY}
        title={t("upcomingExamsPage:navigationHeading")}
      />
      <Box>
